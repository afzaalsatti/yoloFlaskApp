...
def run(
    ...
    for path, im, im0s, vid_cap, s, fps in dataset:
        input_fps=fps
        ...
        #gpu_usage = []
        #for gpu in gpus:
          #  mem_gpu = gpu.memoryUsed / gpu.memoryTotal * 100
            #gpu_usage.append(mem_gpu)

        # Append usage to lists
       ...
       # gpu_usage_list.append(gpu_usage)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    n= int(n)
                    frame_info["detected_Objects"][class_name] = n
                ...
                for *xyxy, conf, cls in reversed(det):
                    axis.append({names[int(cls)]: (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()})
                ...
                detailed_output.insert(len(detailed_output), frame_info)
            # Stream results
        ...
        processing_time += dt[1].dt * 1E3
    end_time = time.time()
    if input_fps > 0:
        for detected_object_class in objects_detected.keys():
            frame_visibility_duration = int(objects_detected.get(detected_object_class))/input_fps
    ...
    #avg_gpu_usage = [sum(x)/len(x) for x in zip(*gpu_usage_list)]
    import platform
    uname = platform.uname()
    server_response = {}
    print("-------------------Strating YoloV5 log--------------------------")
    ...
    print(f'Total Processing Time:{processing_time/1000:.2f}s')
    ...
    print(f'Input Frame rate: {input_fps:.2f} fps')
    print(f'Output Frame rate: {frame_rate:.2f} fps')
    server_response["frames_info"] = {"frames_count":frames_count,"input_frame_rate":input_fps, "output_frame_rate":frame_rate}

    print("-------------------Object Detection--------------------------")
    print("detections:",objects_detected)
    average_conf = sum(accuracy)/len(accuracy)
    print(f'Accuracy conf: {sum(accuracy)/len(accuracy):.2f} %')
    print("Detailed Output For Graphs",detailed_output)
    print("-------------------End YoloV5 log--------------------------")
